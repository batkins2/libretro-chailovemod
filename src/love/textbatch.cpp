...
void TextBatch::uploadVertices(const std::vector<FontMod::GlyphVertex> &vertices, size_t vertoffset)
{
	size_t offset = vertoffset * sizeof(FontMod::GlyphVertex);
	size_t datasize = vertices.size() * sizeof(FontMod::GlyphVertex);
...
void TextBatch::addTextData(const TextData &t)
{
	...
	if (t.align == FontMod::ALIGN_MAX_ENUM)
	...
	if (!newcommands.empty())
	{
		...
		for (FontMod::DrawCommand &cmd : newcommands)
...
FontMod *TextBatch::getFont() const
{
	return font.get();
}
