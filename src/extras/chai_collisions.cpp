        ...
        for (auto &c : characterControllers) {
            for (auto &m : c->meshRef) {
                ...
                if (m == mesh) {
                    ...
                    // printf("Character controller id: %s\n", c->charId.c_str());
                    break;
                }
                if (bb.size() > 0) {
                    break;
...
{  
    for (auto &dw : worlds->worlds) {
        // if (dw.second->future.valid()) {
        //     dw.second->future.wait();
        // }
        // dw.second->future = std::async(std::launch::async, [this, dw, deltaTime]() {
            
        // });
        
        for (auto &cc : this->characterControllers) {
            if (cc->ghostObject->getUserIndex() >= 0) {
                auto stopZ = cc->ghostObject->getUserIndex2();
                auto v = cc->character->getLinearVelocity();
            ...
            cc->character->preStep(dw.second->dynamicsWorld);
            cc->character->playerStep(dw.second->dynamicsWorld, deltaTime);
            ...
            // Perform physics simulation step for this world
            float timeStep = deltaTime;
            float fixedTimeStep = 0.0f;
            dw.second->dynamicsWorld->stepSimulation(timeStep, 10, fixedTimeStep);
        }

        // if (false && self->debugDrawer) {
        //     btVector3 gravity = dw.second->dynamicsWorld->getGravity();
        //     // printf("Gravity: %f, %f, %f\n", gravity.getX(), gravity.getY(), gravity.getZ());   
